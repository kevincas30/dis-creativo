...
export default async function TasksPage({ searchParams }: { searchParams: Promise<{ completed?: string }> }) {
  ...
  const items = await prisma.workItem.findMany({ where: { project: { userId: user.id }, kind: "TASK", completedAt: completed ? { not: null } : null }, include: { project: { select: { name: true } }, period: { select: { label: true } } }, orderBy: [{ dueDate: "asc" }, { createdAt: "asc" }] });
