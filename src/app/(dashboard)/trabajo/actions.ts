...
import { addTaskComment, createTask, deleteTask, deleteTaskComment, editTaskComment, transitionTask, updateTask, type TaskInput } from "@/lib/task-service";
...
export async function transitionWorkTask(id: string, action: "START" | "COMPLETE" | "SEND_REVIEW" | "APPROVE" | "REQUEST_CHANGES" | "REOPEN", comment?: string) { return result(async () => { const context = await requireWorkspaceMembership(); await transitionTask(prisma, workspaceActor(context), id, action, comment); revalidatePath("/", "layout"); return { id }; }); }
export async function deleteWorkTask(id: string) { return result(async () => { const context = await requireWorkspaceMembership(); await deleteTask(prisma, workspaceActor(context), id); revalidatePath("/", "layout"); return { id }; }); }
