...
export default async function WorkPage({ searchParams }: { searchParams: Promise<{ task?: string }> }) {
  ...
  return <div className="relative flex h-full flex-col overflow-y-auto px-4 py-6 sm:px-10 lg:px-16"><DashboardBackground /><div className="mx-auto w-full max-w-6xl pb-12"><WorkPageClient initialTaskId={task ?? null} currentUserId={user.id} role={membership.role} members={members.map(({ user: member }) => ({ id: member.id, name: member.displayName }))} projects={projects.map(project => ({ ...project }))} tasks={tasks.map(task => ({ ...task, startDate: task.startDate?.toISOString() ?? null, dueDate: task.dueDate?.toISOString() ?? null, completedAt: task.completedAt?.toISOString() ?? null, createdAt: task.createdAt.toISOString(), comments: task.comments.map(comment => ({ ...comment, createdAt: comment.createdAt.toISOString(), updatedAt: comment.updatedAt.toISOString() })), activityRecords: task.activityRecords.map(record => ({ ...record, createdAt: record.createdAt.toISOString() })) }))} /></div></div>;
}
