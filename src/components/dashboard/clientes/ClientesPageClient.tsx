...
import { clientCreatedMessage } from "@/components/dashboard/clientes/client-form-utils";
...
export default function ClientesPageClient({ view, clients, members }: { view: View; clients: ClientCardData[]; members: { id: string; name: string }[] }) {
  ...
  function handleCreated(client: { id: string; stage: ClientStage }) {
    setCreateStage(null);
    setNotice(clientCreatedMessage(client.stage));
    const target = client.stage === "PROSPECT" ? "prospects" : "clients";
    router.push(`/clientes?view=${target}#client-${client.id}`);
    router.refresh();
  }
  return <div className="space-y-5">
    ...
    {notice ? <p role="status" aria-live="polite" className="rounded-xl border border-emerald-400/30 bg-emerald-400/10 px-3 py-2 text-sm text-emerald-300">{notice}</p> : null}
