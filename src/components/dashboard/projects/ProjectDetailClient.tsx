  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <div className="relative">
        <Button type="button" variant="ghost" className="h-9 w-9 px-0" aria-label="Más opciones del proyecto" onClick={() => setIsMenuOpen((open) => !open)}>
        ...
        {isMenuOpen ? <div className="bg-surface-solid border-surface-border absolute right-0 z-20 mt-2 w-48 rounded-xl border p-1 shadow-elevated">
          ...
          <button type="button" className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left text-sm text-red-400 hover:bg-red-500/10" onClick={() => { setIsDeleteOpen(true); setIsMenuOpen(false); }}><Trash2 className="h-4 w-4" /> Eliminar proyecto</button>
        </div> : null}
      </div>
