...
export async function getPaymentAccounts(workspaceId: string, clientId?: string): Promise<PaymentAccount[]> {
  const projects = await prisma.project.findMany({ where: { workspaceId, ...(clientId ? { clientId } : {}) }, include: { periods: { orderBy: { startDate: "desc" } }, payments: { orderBy: { paidAt: "desc" } } }, orderBy: { updatedAt: "desc" } });
