...
export async function getWorkView(projectId: string, workspaceId: string) {
  ...
  return {
    project: { ...serializeProject(project), agreedTotal: project.agreedTotal?.toFixed(2) ?? null, depositExpected: project.depositExpected?.toFixed(2) ?? null, currency: project.currency, paymentDueDate: project.paymentDueDate?.toISOString() ?? null, archivedAt: project.archivedAt?.toISOString() ?? null, responsibleId: project.responsibleId },
    ...
    periods: project.periods.map((p) => ({ id: p.id, label: p.label, status: p.status, startDate: p.startDate.toISOString(), dueDate: p.dueDate?.toISOString() ?? null, agreedTotal: p.agreedTotal?.toFixed(2) ?? null, depositExpected: p.depositExpected?.toFixed(2) ?? null, currency: p.currency, paymentDueDate: p.paymentDueDate?.toISOString() ?? null })),
    payments: project.payments.map((p) => ({ id: p.id, periodId: p.periodId, amount: p.amount.toFixed(2), type: p.type, currency: p.currency, paidAt: p.paidAt.toISOString(), note: p.note })),
