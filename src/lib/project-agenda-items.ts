...
export async function getProjectAgendaItems(db: PrismaClient, workspaceId: string): Promise<AgendaItemSnapshot[]> {
  ...
  }), db.workItem.findMany({ where: { workspaceId, kind: "TASK", archivedAt: null, OR: [{ startDate: { not: null } }, { dueDate: { not: null } }] }, select: { id: true, title: true, startDate: true, dueDate: true, project: { select: { id: true, name: true } } } })]);
  ...
  for (const task of tasks) {
    ...
    if (task.startDate && !sameDay) items.push({ ...item(`task-start-${task.id}`, `Inicio · ${task.title}`, task.startDate, base, "PROJECT_START"), projectId: relatedProject?.id ?? null, project: relatedProject, agendaKind: "TASK_START", href: `/trabajo?task=${task.id}` });
    if (task.dueDate) items.push({ ...item(`task-due-${task.id}`, `${sameDay ? "Tarea" : "Entrega"} · ${task.title}`, task.dueDate, base, "PROJECT_DUE"), projectId: relatedProject?.id ?? null, project: relatedProject, agendaKind: "TASK_DUE", href: `/trabajo?task=${task.id}` });
    else if (task.startDate) items.push({ ...item(`task-start-${task.id}`, `Tarea · ${task.title}`, task.startDate, base, "PROJECT_START"), projectId: relatedProject?.id ?? null, project: relatedProject, agendaKind: "TASK_START", href: `/trabajo?task=${task.id}` });
  }
  return items;
}
