...
export type TaskInput = { title: string; description?: string | null; responsibleId?: string | null; projectId?: string | null; periodId?: string | null; startDate?: string | null; startTime?: string | null; dueDate?: string | null; dueTime?: string | null; needsReview?: boolean };
...
function time(input: string | null | undefined, label: string) { const raw = value(input, label); if (raw && !/^([01]\d|2[0-3]):[0-5]\d$/.test(raw)) throw new Error(`${label} no es válida.`); return raw; }
...
async function scopedTask(tx: Parameters<Parameters<PrismaClient["$transaction"]>[0]>[0], actor: WorkspaceActor, id: string) { const task = await tx.workItem.findFirst({ where: { id, workspaceId: actor.workspaceId, kind: { in: ["TASK", "DELIVERABLE"] }, archivedAt: null }, include: { project: true, period: true } }); if (!task) throw new Error("Tarea no encontrada o sin acceso."); return task; }

export async function createTask(db: PrismaClient, actor: WorkspaceActor, input: TaskInput) {
 const title = value(input.title, "El título", true)!; const startDate = date(input.startDate, "La fecha de inicio"); const dueDate = date(input.dueDate, "La fecha de entrega"); const startTime = time(input.startTime, "La hora de inicio"); const dueTime = time(input.dueTime, "La hora de entrega"); if (startDate && dueDate && dueDate < startDate) throw new Error("La entrega no puede ser anterior al inicio.");
 return db.$transaction(async tx => {
   ...
   const task = await tx.workItem.create({ data: { workspaceId: actor.workspaceId, createdById: actor.userId, responsibleId, projectId, periodId, title, description: value(input.description, "La descripción"), startDate, startTime, dueDate, dueTime, needsReview: Boolean(input.needsReview), kind: "TASK", status: "PENDING" } });
...
export async function transitionTask(db: PrismaClient, actor: WorkspaceActor, id: string, action: "START" | "COMPLETE" | "SEND_REVIEW" | "APPROVE" | "REQUEST_CHANGES" | "REOPEN", comment?: string | null) {
 return db.$transaction(async tx => { const task = await scopedTask(tx, actor, id); let next: TaskStatus; let label: string;
   ...
   else if (action === "REOPEN" && task.status === "COMPLETED") { next = "IN_PROGRESS"; label = "Tarea reabierta"; }
...
export async function deleteTask(db: PrismaClient, actor: WorkspaceActor, id: string) {
 if (actor.role !== "ADMIN") throw new Error("Solo un administrador puede eliminar tareas.");
 return db.$transaction(async tx => { await scopedTask(tx, actor, id); await tx.taskComment.deleteMany({ where: { workItemId: id } }); await tx.activityRecord.deleteMany({ where: { workItemId: id } }); await tx.workItem.delete({ where: { id } }); }, { timeout: 30_000, maxWait: 10_000 });
}
